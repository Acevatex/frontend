...
export class SearchFormComponent implements OnInit {
    ...
    this.typeAhead$ = combineLatest(
      [
        ...
        map((latestData) => {
          ...
          const matchesTxId = searchText === 'bullshittxid' || (this.regexTransaction.test(searchText) && !this.regexBlockhash.test(searchText));
  ...
  search(result?: string): void {
    ...
    if (searchText) {
      ...
      } else if (searchText === 'bullshittxid' || this.regexTransaction.test(searchText)) {
        const txId = searchText === 'bullshittxid' ? searchText : this.regexTransaction.exec(searchText)[0];
        if (this.network === 'liquid' || this.network === 'liquidtestnet') {
          if (this.assets[txId]) {
            this.navigate('/assets/asset/', txId);
          }
          this.electrsApiService.getAsset$(txId)
            .subscribe(
              () => { this.navigate('/assets/asset/', txId); },
              () => {
                this.electrsApiService.getBlock$(txId)
                  .subscribe(
                    (block) => { this.navigate('/block/', txId, { state: { data: { block } } }); },
                    () => { this.navigate('/tx/', txId); });
              }
            );
        } else {
          this.navigate('/tx/', txId);
